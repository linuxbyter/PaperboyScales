...
export async function getAgentDetails(
  ...
  return (rows[0] as AgentDetails) ?? null;
}
...
export async function getAgentTransactions(
  ...
  const rows = await sql`
    SELECT * FROM transactions WHERE agent_id = ${agentId} ORDER BY created_at DESC
  `;
...
export async function getAllAgents(): Promise<Profile[]> {
  const rows = await sql`
    SELECT * FROM profiles WHERE role = 'agent' ORDER BY created_at DESC
  `;
...
export async function updateTransactionStatus(
  ...
  const rows = await sql`
    UPDATE transactions SET status = ${status} WHERE id = ${id} RETURNING *
  `;
...
export async function updateProfileStatus(
  ...
  const rows = await sql`
    UPDATE profiles SET status = ${status} WHERE id = ${userId} RETURNING *
  `;
  return (rows[0] as Profile) ?? null;
}
...
export async function getAgentStats(agentId: string) {
  ...
  `;
  return rows[0] as { commission_earned: number; awaiting_receipt: number };
}
...
export async function getAdminStats() {
  const rows = await sql`
    ...
    FROM transactions
  `;
  return rows[0] as { commission_owed: number; in_flight: number; total_volume: number };
}
